...
type innerPointCloudBuilder struct {
	ref *C.struct__draco_point_cloud_builder_t
}

func (m *innerPointCloudBuilder) free() {
	if m.ref != nil {
...
type PointCloudBuilder struct {
	inner *innerPointCloudBuilder
}

func NewPointCloudBuilder() *PointCloudBuilder {
	m := &PointCloudBuilder{inner: &innerPointCloudBuilder{ref: C.draco_new_point_cloud_builder()}}
	runtime.SetFinalizer(m.inner, (*innerPointCloudBuilder).free)
	return m
}

func (m *PointCloudBuilder) Start(size int) {
	C.draco_point_cloud_builder_start(m.inner.ref, C.int(size))
}
...
func (m *PointCloudBuilder) SetAttribute(numPoints int, src interface{}, att GeometryAttrType) error {
	...
	C.draco_point_cloud_set_attribute(C.int(numPoints), m.inner.ref, pt, C.uint(att), C.schar(ncomp), C.uint(dt))
	return nil
}

func (m *PointCloudBuilder) GetPointCloud() *PointCloud {
	pc := &PointCloud{inner: &innerPointCloud{ref: C.draco_point_cloud_builder_get(m.inner.ref)}}
